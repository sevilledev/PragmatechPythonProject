...
class BillingProfileManager(models.Manager):
    ...
    def check_user_otp(self, request):
        user = request.user
        user_otp = request.session.get('user_otp_id')
...
class BillingProfile(models.Model):
    ...
    # butun card lari getirir 
    def get_cards(self):
        return self.card_set.all()

    # default cardlari getirir
    @property
    ...
    def set_inactive_card(self):
        ...
        return cards_qs.filter(active=True).count() #0

# USer yarananda Billing profile yaradir
@receiver(post_save,sender=User)
def user_billing_profile_create(sender,create,instance, *args, **kwargs):
